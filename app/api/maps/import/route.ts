...
function normalizeImportMap(value: any): ImportMapPayload {
  return {
    ...
    mode: String(value.mode || "").trim().toUpperCase(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Array.isArray(body.maps) || body.maps.length === 0) {
      return NextResponse.json({ success: false, message: "등록할 외부 맵을 선택해 주세요." }, { status: 400 });
    }

    const maps: ImportMapPayload[] = body.maps.map(normalizeImportMap);
    if (maps.some((map) => !map.nameEn || !map.mode || !map.sourceKey)) {
      return NextResponse.json({ success: false, message: "외부 맵의 이름, 모드, 식별자 정보가 올바르지 않습니다." }, { status: 400 });
    }

    const sourceKeys = maps.map((map) => map.sourceKey);
    if (new Set(sourceKeys).size !== sourceKeys.length) {
      return NextResponse.json({ success: false, message: "동일한 외부 맵이 중복 선택되었습니다." }, { status: 400 });
    }

    const modes = [...new Set(maps.map((map) => map.mode))];
    const availableModes = await prisma.commonCode.findMany({
      where: { groupCode: MAP_MODE_GROUP_CODE, code: { in: modes }, isUse: true },
      select: { code: true },
    });
    if (availableModes.length !== modes.length) {
      return NextResponse.json({ success: false, message: "MAP_MODE 공통코드에 없는 맵 모드가 포함되어 있습니다." }, { status: 400 });
    }
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      if (existing.length > 0) {
        throw new ImportValidationError("이미 등록된 외부 맵이 포함되어 있습니다.");
      }

      return Promise.all(maps.map((map) => tx.mapItem.create({
        data: {
          name: map.nameEn,
          nameEn: map.nameEn,
          mapType: map.mode,
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      return NextResponse.json({ success: false, message: "다른 요청으로 이미 등록된 외부 맵이 포함되어 있습니다." }, { status: 409 });
    }
    return NextResponse.json({ success: false, message: "외부 맵을 등록하지 못했습니다." }, { status: 500 });
  }
}
