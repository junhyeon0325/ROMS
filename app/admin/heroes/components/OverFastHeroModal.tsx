...
export default function OverFastHeroModal({
  isOpen,
  isSaving,
  isExternalLoading,
  externalHeroes,
  ...
  const filteredHeroes = useMemo(() => {
    ...
    return externalHeroes.filter(
      (hero) =>
        !query ||
        [hero.nameEn, hero.role, hero.subrole].some((value) =>
  ...
  const selectedHeroes = useMemo(
    () =>
      externalHeroes.filter((hero) =>
        selectedExternalKeys.includes(hero.sourceKey),
      ),
    [externalHeroes, selectedExternalKeys],
  );
  ...
  const selectableHeroes = useMemo(
    ...
    [filteredHeroes, registeredByNameEn, registeredBySourceKey],
  );
  const isAllSelectableSelected =
    selectableHeroes.length > 0 &&
    selectableHeroes.every((hero) =>
      selectedExternalKeys.includes(hero.sourceKey),
    );
    ...
    () => [
      {
        ...
        header: (
          <input
            ...
            disabled={!selectableHeroes.length}
            onChange={() => onSelectAllHeroes(selectableHeroes)}
        ...
        render: (hero) => (
          <input
            type="checkbox"
            checked={selectedExternalKeys.includes(hero.sourceKey)}
            disabled={
              registeredBySourceKey.has(hero.sourceKey) ||
              registeredByNameEn.has(hero.nameEn)
            }
            onClick={(event) => event.stopPropagation()}
            onChange={() => onSelectHero(hero)}
            className="h-3.5 w-3.5 rounded border-slate-300 text-[#f99e1a] accent-[#f99e1a] disabled:cursor-not-allowed disabled:opacity-40"
            aria-label={`${hero.nameEn} 선택`}
          />
        ),
      },
      {
  ...
  return (
    <AdminModal
      ...
      title="OverFast 영웅 목록 불러오기"
      description="등록할 영웅을 여러 개 선택한 뒤 저장하면 영웅 마스터에 즉시 등록됩니다."
      maxWidth="4xl"
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={onSave}
              disabled={!selectedHeroes.length || isSaving}
            ...
            >
              {isSaving ? "저장 중..." : `DB 저장 (${selectedHeroes.length}명)`}
            </button>
          </div>
        </div>
      }
    >
      <AdminSearchInput
        label="외부 영웅 검색"
        placeholder="영문 이름, 역할군 또는 하위 역할 검색..."
        value={search}
        onChange={setSearch}
      />
      <div className="h-[420px] shrink-0">
        <AdminTable
          ...
          emptyTitle="외부 영웅 목록을 불러오는 중입니다."
