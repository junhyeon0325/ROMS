...
export default function OverFastHeroModal({ isOpen, isSaving, isExternalLoading, externalHeroes, selectedExternalKeys, registeredBySourceKey, registeredByNameEn, onClose, onSelectHero, onSelectAllHeroes, onSave, heroRoleNameByCode }: OverFastHeroModalProps) {
  ...
  const columns: AdminTableColumn<ExternalHero>[] = useMemo(() => [
    ...
    { key: "role", header: "역할군", width: "w-28", render: (hero) => <AdminBadge variant="neutral">{heroRoleNameByCode.get(hero.role) || hero.role}</AdminBadge> },
  ...
  ], [heroRoleNameByCode, isAllSelectableSelected, onSelectAllHeroes, onSelectHero, registeredByNameEn, registeredBySourceKey, selectableHeroes, selectedExternalKeys]);
