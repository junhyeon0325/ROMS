...
> **기준 산출물**: ROMS 요구사항정의서 v1.4 (`CM-001` ~ `AD-004`)  
...
│   │
│   ├── 1.1 메인 대시보드 (SCR-001)
│   │   ├── 1.1.1 상단 스트리머 통합 검색바 (US-001)
│   │   ├── 1.1.2 중앙 바로가기 메뉴 (시즌1~4, 라이벌클래시, 통합 랭킹)
│   │   └── 1.1.3 하단 전체 시즌 통합 13종 리더보드 지표 카드 (CM-001)
│   │
│   ├── 1.2 스트리머 상세 (SCR-002)
...
└── [2.0 Admin Portal (관리자 영역)] - ADMIN 권한 전용 (Protected)
    ...
    │
    ├── 2.2 대회/시즌 및 팀 관리 (SCR-005)
    │   ├── 2.2.1 시즌 등록 (대회명, 기간, Supabase Storage 로고 업로드) (AD-001)
    │   └── 2.2.2 팀 생성 (팀명, Supabase Storage 엠블럼 업로드) (AD-001)
    │
    ├── 2.3 선수 배정 및 포지션 관리 (SCR-006)
    │   └── 2.3.1 스트리머 배정 (시즌 참가 팀 지정, 포지션 TANK/DAMAGE/HEALER 설정) (AD-002)
    │
    └── 2.4 경기 결과 & 세부 스탯 입력 (SCR-007)
        ├── 2.4.1 매치 생성 (대회 단계, 경기 일시, 팀A vs 팀B) (AD-003)
        ├── 2.4.2 세트 입력 (세트 번호, 맵 선택, 세트 승리팀, 경기 시간, VOD URL) (AD-003)
        ├── 2.4.3 영웅 밴 지정 (팀별 밴 영웅 선택) (AD-003)
        └── 2.4.4 선수별 세부 스탯 입력 (K/D/A, 피해/치유/경감량, MVP 토글) (AD-003)
```
...
| :--- | :--- | :--- | :---: | :--- | :--- |
| `SCR-001` | **메인 대시보드** | `/` | 전체 (Public) | 상단 통합 검색바, 중앙 바로가기 버튼, 하단 13종 통합 데이터 카드 표출 및 상세화면 이동 링크 | `CM-001`, `US-001` |
...
| `SCR-007` | **관리자 - 경기 스탯 입력**| `/admin/match` | 관리자 (ADMIN) | 매치/세트 결과 생성, 맵 및 영웅 밴 선택, 선수별 K/D/A, 피해/치유량 및 MVP 설정 | `AD-003`, `AD-004` |
