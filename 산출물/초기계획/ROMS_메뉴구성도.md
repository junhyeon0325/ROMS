...
> **문서 버전**: v1.3 (스트리머(선수) 등록 메뉴를 공통관리에서 대회/시즌, 팀 및 선수 관리로 이동)  
> **작성일**: 2026-07-30  
> **프로젝트명**: ROMS (오버워치 e스포츠 아카이브 및 대시보드 시스템)  
> **기준 산출물**: ROMS 요구사항정의서 v1.7 (`CM-001` ~ `AD-004`)  
...
└── [2.0 Admin Portal (관리자 영역)] - ADMIN 권한 전용 (Protected)
    ...
    │
    ├── 2.2 공통 마스터 관리 (영웅/맵) (SCR-009)
    │   ├── 2.2.1 영웅 등록 및 관리 (영웅명, 역할군 TANK/DAMAGE/HEALER, 영웅 아이콘) (AD-004)
    │   └── 2.2.2 맵 등록 및 관리 (맵 이름, 맵 전형 점령/화물/혼합/밀기, 맵 이미지) (AD-004)
    │
    ├── 2.3 대회/시즌, 선수 및 팀 관리 (SCR-005)
    │   ├── 2.3.1 시즌 등록 (대회명, 기간, Supabase Storage 로고 업로드) (AD-001)
    │   ├── 2.3.2 스트리머(선수) 등록 (선수명, 방송 닉네임, 프로필 이미지 URL, 채널 URL) (AD-004)
    │   └── 2.3.3 팀 생성 (팀명, Supabase Storage 엠블럼 업로드) (AD-001)
    │
...
| `SCR-005` | **관리자 - 시즌/선수/팀 관리** | `/admin/season` | 관리자 (ADMIN) | 대회/시즌 생성, 스트리머(선수) 등록/수정, 팀 생성 (Supabase Storage 연동) | `AD-001`, `AD-004` |
...
| `SCR-009` | **관리자 - 공통 마스터 관리** | `/admin/common` | 관리자 (ADMIN) | 영웅 및 맵 마스터 데이터 등록 관리 | `AD-004` |
...
flowchart TD
    ...
    subgraph AdminGroup [관리자 전용 제어 영역]
        SCR005[SCR-005: 시즌/선수/팀 관리]
        SCR006[SCR-006: 선수 배정]
        SCR007[SCR-007: 경기 스탯 입력]
        SCR009[SCR-009: 영웅/맵 마스터 관리]
    end
```
