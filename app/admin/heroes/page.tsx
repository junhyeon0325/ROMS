...
import { importHeroes, saveHero } from "@/lib/heroes/heroClient";
...
export default function AdminHeroesPage() {
  ...
  const registeredBySourceKey = useMemo(
    () =>
      new Map(
        heroes
          .filter((hero) => hero.sourceKey)
          .map((hero) => [hero.sourceKey!, hero]),
      ),
    [heroes],
  );
  const registeredByNameEn = useMemo(
    () => new Set(heroes.map((hero) => hero.nameEn)),
    [heroes],
  );
  ...
  const applySavedHero = (saved: HeroItem) => {
    setHeroes((current) =>
      current.some((hero) => hero.id === saved.id)
        ? current.map((hero) => (hero.id === saved.id ? saved : hero))
        : [saved, ...current],
    );
  ...
  const handleSelectExternalHero = (hero: ExternalHero) => {
    if (
      registeredBySourceKey.has(hero.sourceKey) ||
      registeredByNameEn.has(hero.nameEn)
    ) {
      showFeedback("이미 등록된 외부 영웅입니다.");
      return;
    }
    setSelectedExternalKeys((current) =>
      current.includes(hero.sourceKey)
        ? current.filter((key) => key !== hero.sourceKey)
        : [...current, hero.sourceKey],
    );
  };
  ...
  const handleSelectAllExternalHeroes = (visibleHeroes: ExternalHero[]) => {
    ...
    const isAllSelected = visibleKeys.every((key) =>
      selectedExternalKeys.includes(key),
    );
    setSelectedExternalKeys((current) =>
      isAllSelected
  ...
  const handleSaveExternalHeroes = async () => {
    const selectedHeroes = externalHeroes.filter((hero) =>
      selectedExternalKeys.includes(hero.sourceKey),
    );
    ...
    await mutateHero(() => importHeroes(selectedHeroes), {
      successMessage: (saved?: HeroItem[]) =>
        `${saved?.length || 0}명의 영웅을 DB에 등록했습니다.`,
      errorMessage: "외부 영웅 등록에 실패했습니다.",
      onSuccess: (saved?: HeroItem[]) => {
        if (!saved?.length) return;
        saved.forEach(applySavedHero);
  ...
  const handleSave = async () => {
    ...
    await mutateHero(() => saveHero(form, selectedHeroId), {
      successMessage: selectedHeroId
        ? `[${form.nameKr}] 영웅 정보를 수정했습니다.`
        : `[${form.nameKr}] 영웅을 등록했습니다.`,
  ...
  return (
    <section className="h-full min-h-0 flex flex-col">
          ...
          <AdminCard
            ...
            actions={
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={handleOpenOverFastModal}
            ...
            <div className="space-y-4 overflow-y-auto pr-1.5 custom-scrollbar">
              ...
              <AdminFormField label="역할군" required>
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs focus:border-[#f99e1a] focus:outline-none dark:border-slate-700 dark:bg-slate-800"
                >
                  <option value="TANK">돌격 (Tank)</option>
                  <option value="DAMAGE">공격 (Damage)</option>
      ...
      <OverFastHeroModal
        ...
        externalHeroes={externalHeroes}
        selectedExternalKeys={selectedExternalKeys}
        registeredBySourceKey={registeredBySourceKey}
        registeredByNameEn={registeredByNameEn}
        onClose={() => setIsOverFastModalOpen(false)}
        onSelectHero={handleSelectExternalHero}
        onSelectAllHeroes={handleSelectAllExternalHeroes}
        onSave={handleSaveExternalHeroes}
      />
    </section>
  );
}
