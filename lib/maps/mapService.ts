...
export async function createMap(
  payload: ReturnType<typeof normalizeMapPayload>,
) {
  return formatMapDto(
    await prisma.mapItem.create({ data: toMapData(payload) }),
  );
}
export async function updateMap(
  ...
  return formatMapDto(
    await prisma.mapItem.update({
      where: { id: BigInt(id) },
      data: toMapData(payload),
    }),
  );
}
...
export async function importMaps(values: unknown[]) {
  if (!values.length)
    throw new MapImportValidationError("등록할 신규 맵을 선택해 주세요.");
  const maps = values.map((value) =>
    normalizeImportMap((value ?? {}) as Record<string, unknown>),
  );
  if (maps.some((map) => !map.nameEn || !map.mode || !map.sourceKey))
    throw new MapImportValidationError(
      "신규 맵의 이름, 모드, 식별자 정보가 올바르지 않습니다.",
    );
  const sourceKeys = maps.map((map) => map.sourceKey);
  if (new Set(sourceKeys).size !== sourceKeys.length)
    throw new MapImportValidationError("동일한 신규 맵이 중복 선택되었습니다.");
  ...
  if (availableModes.length !== modes.length)
    throw new MapImportValidationError(
      "MAP_MODE 공통코드에 없는 맵 모드가 포함되어 있습니다.",
    );
  try {
