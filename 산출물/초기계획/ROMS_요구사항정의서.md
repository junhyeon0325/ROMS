...
| **AD**    | 관리자 (Admin Portal)   | 운영자를 위한 대회/시즌/팀/선수 관리, 공통 마스터(영웅/맵/스트리머) 관리, 선수 배정, 경기 세트 점수 및 상세 스탯 입력 |
...
| 요구사항 ID | 요구사항명                  | 상세 설명                                                                                                                       | 우선순위 | 관련 엔터티 및 컬럼                                                                                                                                              | 비고   |
| :---------- | :-------------------------- | :------------------------------------------------------------------------------------------------------------------------------ | :------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----- |
| `AD-001`    | **시즌 및 팀/로스터 관리**  | • 시즌(대회명, 기간, 로고) 생성<br>• **팀 생성 및 로스터 배정**: 팀 생성(팀명, Supabase Storage 엠블럼 업로드) 시 등록된 포지션별 선수 풀에서 팀원을 직접 픽(Pick)하여 배정 |  **상**  | • `Season`<br>&nbsp;&nbsp;(`name`, `logo_url`, `start_date`, `end_date`)<br>• `Team`<br>&nbsp;&nbsp;(`name`, `emblem_url`)<br>• `SeasonTeamMember`<br>&nbsp;&nbsp;(`season_team_id`, `streamer_id`, `position`) | 관리자 (`/admin/season`) |
| `AD-002`    | **선수 로스터 배정 및 관리**| • 팀 생성 및 편집 시 스트리머를 해당 팀의 로스터로 지정 (선수 등록 시 설정된 포지션 `TANK`, `DAMAGE`, `HEALER` 자동 매칭 및 드래프트 결과 반영) |  **상**  | • `SeasonTeamMember`<br>&nbsp;&nbsp;(`season_team_id`, `streamer_id`, `position`)                                                                                | 관리자 (`/admin/season`) |
| `AD-003`    | **매치 관리, 게임 흐름 스탯 입력 & 조회** | • **매치 선행 생성 및 목록 관리**: 시즌별 토너먼트 매치 사전 생성 및 목록 제어 (`/admin/match`) <br>• **게임 흐름 기반 상세 기록 입력**: 매치 선택 시 이동 (`/admin/match/[id]`)하여 세트 입력 ➔ 영웅 밴 선택 ➔ 선수별 세부 스탯(K/D/A, 피해/치유/경감, MVP) 순차적 기록<br>• **경기 결과 & 스탯 통합 조회**: 전체 입력 결과 검수 및 필터링 검색 (`/admin/match/results`) |  **상**  | • `Match`, `MatchSet`, `PlayerSetStat`, `HeroBan`<br>&nbsp;&nbsp;(`tournament_stage`, `match_date`, `set_number`, `kills`, `deaths`, `assists`, `damage`, `healing`, `mitigated_damage`, `is_mvp`) | 관리자 (`/admin/match`, `/admin/match/[id]`, `/admin/match/results`) |
| `AD-004`    | **공통 마스터 관리 (스트리머, 영웅, 맵)** | • **스트리머(선수) 마스터 관리**: 선수명, 방송 닉네임, **대표 포지션(`TANK`, `DAMAGE`, `HEALER`)**, Supabase Storage 프로필 이미지 URL, 채널 URL 등록/수정<br>• **영웅/맵 마스터 관리**: 오버워치 영웅 및 맵 마스터 데이터 등록/수정 | **상** | • `Streamer`<br>&nbsp;&nbsp;(`name`, `nickname`, `profile_image_url`, `position`)<br>• `MapItem`<br>&nbsp;&nbsp;(`name`, `map_type`, `image_url`) | 관리자 (`/admin/common`) |
...
| :---------------------------- | :------------------------------------- | :--------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------ |
| `CM-001`                      | 메인 대시보드 (`/`)                    | `Streamer`, `Season`, `PlayerSetStat`, `Match`             | `Streamer.profile_image_url`, `Streamer.position`, `PlayerSetStat.kills/damage/healing/mitigated_damage`, `Match.winner_team_id` |
| `CM-002`                      | 로그인 페이지 (`/auth/login`)          | `User`                                                     | `User.email`, `User.password_hash`, `User.role`                                                               |
| `US-001` ~ `US-006`           | 스트리머 상세 (`/streamer/[id]`)       | `Streamer`, `SeasonTeamMember`, `PlayerSetStat`, `HeroBan` | `Streamer.name/profile_image_url/position`, `PlayerSetStat.kills/deaths/assists/damage/healing`, `HeroBan.hero_id` |
...
| `AD-001`, `AD-002`            | 관리자 시즌/팀/로스터 관리 (`/admin/season`) | `Season`, `Team`, `SeasonTeam`, `SeasonTeamMember`  | `Season.name/logo_url`, `Team.name/emblem_url`, `SeasonTeamMember.season_team_id/streamer_id/position`         |
| `AD-003`                      | 관리자 매치 관리 & 기록 입력 (`/admin/match`, `/admin/match/[id]`) | `Match`, `MatchSet`, `PlayerSetStat`, `HeroBan` | `tournament_stage`, `match_date`, `set_number`, `kills/deaths/assists/damage/healing/mitigated_damage/is_mvp` |
| `AD-003`                      | 관리자 경기 결과 & 스탯 조회 (`/admin/match/results`) | `Match`, `MatchSet`, `PlayerSetStat`               | `tournament_stage`, `winner_team_id`, `kills/deaths/assists`                                                 |
| `AD-004`                      | 관리자 공통 마스터 관리 (`/admin/common`) | `Streamer`, `MapItem`, `HeroBan`                           | `Streamer.name/nickname/position`, `MapItem.name/map_type/image_url`                                         |
