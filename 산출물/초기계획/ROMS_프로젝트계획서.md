...
> **문서 버전**: v1.5 (관리자 공통관리 메뉴 신설 및 스트리머/영웅/맵 등록 요구사항 반영)  
> **작성일**: 2026-07-29  
> **기준 산출물**: ROMS 프로젝트 요구사항정의서 v1.6  
...
2. **운영 효율화**: 관리자(ADMIN) 전용 페이지를 통해 공통 마스터(스트리머/영웅/맵) 등록, 대회 생성, 팀 및 선수 배정, 경기 세트 점수 및 선수별 상세 스탯 입력을 신속하고 정확하게 처리.
...
* **공통 마스터 등록 및 관리 (`AD-004`)**:
  * 스트리머(선수) 프로필 및 방송/유튜브 채널 URL 등록, 영웅 마스터 및 맵 마스터 데이터 등록/수정
* **대회 정보 등록 (`AD-001`)**:
  * 시즌(대회명, 기간, 로고) 등록 및 참가 팀 생성, 팀 엠블럼 업로드 (**Supabase Storage** 연동)
* **선수 배정 관리 (`AD-002`)**:
