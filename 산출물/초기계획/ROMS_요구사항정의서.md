...
> **문서 버전**: v1.4 (메인 대시보드 지표 13종 상세화 및 엔터티/우선순위 체계 개정)  
> **작성일**: 2026-07-30  
...
| **AD** | 관리자 (Admin Portal) | 운영자를 위한 시즌 등록, 팀/선수 배정, 경기 세트 점수 및 상세 스탯 입력 |
...
| 우선순위 | 구분 정의 | 적용 대상 |
| :---: | :--- | :--- |
| **상 (High)** | 필수 핵심 기능 (시스템 가동 및 서비스 제공을 위해 반드시 구현되어야 함) | 메인 대시보드, 핵심 전적 조회, 로그인, 경기 데이터 입력 등 |
...
| :--- | :--- | :--- | :---: | :--- | :--- |
| `CM-001` | **메인 대시보드 구성** | **1. 상단 영역**: 스트리머명을 입력할 수 있는 통합 검색바 배치<br>**2. 중앙 메뉴 영역**: 시즌별(시즌1~4, 라이벌클래시) 및 랭킹 바로가기 버튼 배치<br>**3. 하단 대시보드 영역**: 전체 시즌 통합 데이터(13종 리더보드 지표 카드) 제공<br>**4. 대시보드 표출 데이터 (13종)**:<br>&nbsp;&nbsp;• **우승 횟수 Top5**<br>&nbsp;&nbsp;• **단일 세트 최다**: 최다처치, 최다도움, 최다죽음, 최다피해, 최다치유, 최다경감<br>&nbsp;&nbsp;• **누적 통계**: 누적최다처치, 누적최다도움, 누적최다죽음, 누적최다피해, 누적최대치유, 누적최다경감<br>**5. 카드 UI 세부 구성**: 각 카드 내 데이터 숫자 값과 선수 대표 프로필 이미지가 표출되며, **이미지 클릭 시 해당 선수 상세화면(`/streamer/[id]`)으로 이동** | **상** | • `Streamer`<br>&nbsp;&nbsp;(`id`, `name`, `nickname`, `profile_image_url`)<br>• `Season`<br>&nbsp;&nbsp;(`id`, `name`, `logo_url`)<br>• `PlayerSetStat`<br>&nbsp;&nbsp;(`kills`, `deaths`, `assists`, `damage`, `healing`, `mitigated_damage`)<br>• `Match`<br>&nbsp;&nbsp;(`winner_team_id`) | 메인 화면 핵심 구성 |
...
| `AD-004` | 맵 및 영웅 마스터 관리 | • 오버워치 맵(점령/화물/혼합/밀기) 및 영웅 픽 목록 마스터 데이터 관리 | **중** | • `MapItem`<br>&nbsp;&nbsp;(`name`, `map_type`, `image_url`) | 관리자 |
